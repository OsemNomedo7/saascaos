  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ marginBottom: 24, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ ...mono, fontSize: '1rem', fontWeight: 700, color: '#ff4400', letterSpacing: '0.1em', margin: '0 0 4px' }}>{'// CHAT — #' + room.toUpperCase()}</h1>
          <p style={{ ...mono, fontSize: '0.65rem', color: '#5a2a10' }}>Controle total sobre mensagens do chat</p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => { refetch(); refetchStats(); }} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 12px', background: 'rgba(255,68,0,0.06)', border: '1px solid rgba(255,68,0,0.15)', borderRadius: 4, cursor: 'pointer', ...mono, fontSize: '0.62rem', color: '#7a3a20' }}>
      ...
      {showClearConfirm && (
        <div style={{ marginBottom: 16, padding: '14px 18px', background: 'rgba(255,0,0,0.08)', border: '1px solid rgba(255,0,0,0.25)', borderRadius: 6 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={() => setShowClearConfirm(false)} style={{ padding: '6px 14px', background: 'transparent', border: '1px solid rgba(255,68,0,0.15)', borderRadius: 4, cursor: 'pointer', ...mono, fontSize: '0.62rem', color: '#7a3a20' }}>CANCELAR</button>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3" style={{ marginBottom: 20 }}>
        {[
          { label: 'TOTAL MSGS', value: stats.total, icon: <BarChart2 style={{ width: 14, height: 14 }} />, color: '#ff4400' },
          ...
          <div key={s.label} style={{ padding: '12px 16px', background: '#0a0a0a', border: `1px solid ${s.color}22`, borderRadius: 6 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
              <span style={{ color: s.color }}>{s.icon}</span>
              <span style={{ ...mono, fontSize: '0.55rem', color: '#5a2a10', letterSpacing: '0.15em' }}>{s.label}</span>
            </div>
      ...
      <div style={{ background: '#0a0a0a', border: '1px solid rgba(255,68,0,0.12)', borderRadius: 6, overflow: 'hidden' }}>
        <div style={{ padding: '10px 14px', borderBottom: '1px solid rgba(255,68,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ ...mono, fontSize: '0.62rem', color: '#ff4400' }}>MENSAGENS — página {page}</span>
          <span style={{ ...mono, fontSize: '0.58rem', color: '#5a2a10' }}>{messagesData?.total || 0} total</span>
        </div>
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(255,68,0,0.1)' }}>
                {['TIPO', 'AUTOR', 'MENSAGEM', 'REAÇÕES', 'DATA', 'AÇÃO'].map(h => (
                  <th key={h} style={{ padding: '9px 12px', textAlign: 'left', ...mono, fontSize: '0.55rem', color: '#7a3a20', letterSpacing: '0.12em', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                <tr><td colSpan={6} style={{ padding: '24px', textAlign: 'center', ...mono, fontSize: '0.65rem', color: '#3a1800' }}>Carregando...</td></tr>
              ) : messages.length === 0 ? (
                <tr><td colSpan={6} style={{ padding: '24px', textAlign: 'center', ...mono, fontSize: '0.65rem', color: '#3a1800' }}>Nenhuma mensagem</td></tr>
              ) : messages.map(m => (
                <tr key={m._id} style={{ borderBottom: '1px solid rgba(255,68,0,0.05)', background: m.isDeleted ? 'rgba(255,0,0,0.03)' : 'transparent', opacity: m.isDeleted ? 0.5 : 1 }}>
                  <td style={{ padding: '9px 12px' }}>{typeIcon(m.type)}</td>
                  <td style={{ padding: '9px 12px' }}>
                    <p style={{ ...mono, fontSize: '0.65rem', color: m.author?.role === 'admin' ? '#ff7755' : '#d08060', margin: 0, whiteSpace: 'nowrap' }}>{m.author?.name}</p>
                    <p style={{ ...mono, fontSize: '0.55rem', color: '#5a2a10', margin: 0 }}>{m.author?.email}</p>
                  </td>
                  <td style={{ padding: '9px 12px', maxWidth: 300 }}>
                    {m.isDeleted ? (
                      <span style={{ ...mono, fontSize: '0.6rem', color: '#5a2a10', fontStyle: 'italic' }}>— mensagem deletada —</span>
                    ) : m.content ? (
                      <p style={{ ...mono, fontSize: '0.68rem', color: '#c08060', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.content}</p>
                    ) : m.mediaFileName ? (
                    ...
                    ) : <span style={{ color: '#3a1800', ...mono, fontSize: '0.6rem' }}>—</span>}
                  </td>
                  <td style={{ padding: '9px 12px' }}>
                    {m.reactions && m.reactions.length > 0 ? (
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                        {m.reactions.map(r => (
                          <span key={r.emoji} style={{ ...mono, fontSize: '0.6rem', background: 'rgba(255,68,0,0.08)', padding: '1px 5px', borderRadius: 10, color: '#c08060' }}>
                  ...
                  <td style={{ padding: '9px 12px', ...mono, fontSize: '0.6rem', color: '#5a2a10', whiteSpace: 'nowrap' }}>
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 12, justifyContent: 'center' }}>
          {Array.from({ length: Math.min(totalPages, 7) }, (_, i) => {
            const p = i + 1;
            return (
              <button key={p} onClick={() => setPage(p)} style={{ padding: '5px 10px', background: page === p ? 'rgba(255,68,0,0.15)' : 'rgba(255,68,0,0.05)', border: `1px solid ${page === p ? 'rgba(255,68,0,0.35)' : 'rgba(255,68,0,0.1)'}`, borderRadius: 4, cursor: 'pointer', ...mono, fontSize: '0.62rem', color: page === p ? '#ff4400' : '#7a3a20' }}>
                {p}
              </button>
            );
          })}
          {totalPages > 7 && <span style={{ ...mono, fontSize: '0.62rem', color: '#5a2a10' }}>... {totalPages}</span>}
        </div>
      )}
    </div>
  );
